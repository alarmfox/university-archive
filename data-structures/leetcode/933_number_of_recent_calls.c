...
int recentCounterPing(RecentCounter *rc, int t) {
  ...
  while (rc->start < rc->end && rc->elems[rc->start] < (t - INTERVAL_SIZE))
    rc->start++;

  rc->elems[rc->end++] = t;

  return rc->end - rc->start;
}
...
int main(void) {
  ...
  n = recentCounterPing(rc, 1);
  printf("ping(1): %d\n", n);
  n = recentCounterPing(rc, 100);
  printf("ping(100): %d\n", n);
  n = recentCounterPing(rc, 3001);
  printf("ping(3001): %d\n", n);
  n = recentCounterPing(rc, 3002);
  printf("ping(3002): %d\n", n);
