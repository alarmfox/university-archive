...
double voutput_C[SIZE] __attribute__((aligned (32)));
	
inline void complexArrayMult( double * __restrict__ p1, double * __restrict__ p2, double * __restrict__ p3, size_t size ){
//inline void complexArrayMult( double * p1, double * p2, double * p3, size_t size ){
...
inline void complexArrayMult_C( double * __restrict__ p1, double * __restrict__ p2, double * __restrict__ p3, size_t size ){
...
void kernel(){
	...
	// initialize here vinput1 and vinput2 ....
	srand(34281);
    for(int j=0; j<SIZE; j++){
		long long unsigned ll = rand();
		ll = (ll << 15) ^ rand();
		ll = (ll << 15) ^ rand();
		ll = (ll << 15) ^ rand();
		ll = (ll << 15) ^ rand();
    ...
    if (1) for(int j=0; j<REP; j++){
		complexArrayMult( vinput1, vinput2, voutput, SIZE );
	}


    if (1) for(int j=0; j<REP; j++){
		complexArrayMult_C( vinput1, vinput2, voutput_C, SIZE );
	}

    if (0) for(int j=0; j<16; j++){
		printf("\n\n  voutput[%d] = %f \nvoutput_C[%d] = %f",j,voutput[j],j,voutput_C[j]);
