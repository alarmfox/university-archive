...
__global__ void reduction(float* in, float* out, int size, clock_t* tm) {
	...
	ldata[tx] = (x < size) ? in[x] : 0;
	ldata[tx+stride] = ((x+stride) < size) ? in[x+stride] : 0;
		
	for( ; stride>=1; stride >>=1 ) {
		__syncthreads();
		if ( (tx<stride) ) {
			ldata[tx] += ldata[tx+stride];
		}		
	}
	if (tx==0) {
		out[blockIdx.x] = ldata[tx];
	}
...
int main(){
	...
	do{
		...
		reduction<<<num_blocks, THREADSPERBLOCK, 2*THREADSPERBLOCK*sizeof(float)>>>(in_d, out_d, SIZE, tm_d);
